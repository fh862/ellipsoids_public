...
from analysis.ellipses_tools import (
    GegenfurtnerEll,
    UnitCircleGenerate,
    convert_2Dcov_to_points_on_ellipse,
...
from plotting.color_spaces_plotting import (
    DKLRelatedSpacesVisualization,
    PlotGegenfurtner,
)
...
if flag_running_on_hpc:
    ...
    file_name_fits = (
        f"Fitted_ColorDiscrimination_4dExpt_Isoluminant plane_sub{subN}"
...
if key_pred not in vars_dict:
    ...
    comp_Gegenfurtner = {
        ...
        for name in [
            ...
            "fine_ell_grey_DKL",
            "fine_ell_grey_sDKL",
            "covMat_around_ref_DKL",
            "covMat_around_ref_sDKL",
            "fine_ell_W",
            "fine_ell_DKL",
            "fine_ell_sDKL",
        ]
...
if not flag_running_on_hpc:
    ...
    Gegenfurtner_vis = DKLRelatedSpacesVisualization(
        model_pred_Wishart,
        comp_Gegenfurtner,
        color_thres_data,
        pltSettings_base,
    ...
    Gegenfurtner_vis.plot_Gegenfurtner_comparison(
        grid_sDKL,
        grid_Wishart,
        fine_ell_sDKL_grid,
        settings=plt_st,
    ...
    fig1.savefig(
        os.path.join(output_figDir_fits, f"Comp_GegenfurtnerEllipses_sub{subN}_W.pdf"),
        format="pdf",
        bbox_inches="tight",
    )
