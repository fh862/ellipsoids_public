...
from plotting.modelperf_plotting import (
    NFoldsCrossValidationVisualization,
...
if hyper_param == "DecayRate":
    hyper_param_arr = np.concatenate([np.array([0.15]), np.arange(0.2, 1.05, 0.05)])
        ...
        for f in range(total_folds):
            ...
            while nSuccess < nRepeats:
                ...
                try:
                    # print out counters
                    print(
                        f"{hyper_param}: {val:.{decimals}f}; fold: {f + 1}; "
                ...
                # print the results
                print(
                    f"nPL (training data): {nPL_training_data:.4f}; "
...
if not flag_running_on_hpc:
    ...
    for d, val in enumerate(hyper_param_arr):
        ...
        full_path_set_d = full_path_set.replace(
            f"varying{hyper_param}{hyper_param_arr[0]:.{decimals}f}",
    ...
    # Generate plot
    nLL_vis.plot_nLL_varying_hyper_param(
        hyper_param_arr,
        nLL_training_avg,
        nLL_test_avg,
        nLL_training_CI,
...
if not flag_running_on_hpc:
    ...
    for n, val in enumerate(hyper_param_arr):
        ...
        if np.abs(val - base_kwargs[sweep_arg]) < 1e-10:
            ax.plot(
                ...
                lw=1,
                ls="--",
                label=rf"Our choice: ${hp_symbol}$={val:.{decimals}f}",
            )
    ...
    cbar.set_label(rf"Hyperparameter ${hp_symbol}$")
