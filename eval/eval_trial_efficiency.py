...
from analysis.utils_load import load_expt_data, extract_between_patterns
...
from plotting.modelperf_plotting import EvaluateTrialEfficiencyVisualization, \
    PltVaryingTrialsNumbersSettings
...
input_fileDir_fits = os.path.join(base_dir, 'META_analysis','ModelFitting_DataFiles',
                                 f'{ndims*2}dTask','CIE','sub1',
                                 'decayRate0.5' if ndims == 2 else 'decayRate0.4')

if ndims == 2:
    sim_file_name = 'Fitted_byWishart_Isoluminant plane_4DExpt_300_300_300_5100_'+\
        'AEPsychSampling_EAVC_decayRate0.5_nBasisDeg5_sub1.pkl'
elif ndims == 3:           
    sim_file_name = 'Fitted_byWishart_ellipsoids_6DExpt_1500_1500_1500_25500_'+\
...
if ndims == 2:
    gt_fileDir_fits = os.path.join(base_dir, 'ELPS_analysis','ModelFitting_DataFiles',
                                   f'{ndims}D_oddity_task','Isoluminant_plane' \
                                       if flag_running_on_hpc else 'Isoluminant plane') 
                                    #in hpc: Isoluminant_plane
                                    #local: Isoluminant plane
    gt_file_name = 'Fitted_isothreshold_Isoluminant plane_CIE1994_sim18000total_'+\
                   'samplingNearContour_jitter0.3_seed0_bandwidth0.005_decay0.4_oddity.pkl'
elif ndims == 3:
    gt_fileDir_fits = os.path.join(base_dir, 'ELPS_analysis','ModelFitting_DataFiles',
                                   f'{ndims}D_oddity_task', 'CIE1994')
    gt_file_name = 'Fitted_isothreshold_ellipsoids_sim240perCond_samplingNearContour_'+\
...
_, _, nTrials_edge, nTrials_central = \
...
if desired_edge_percentage is None:
    ...
    print(f"The edge-to-total proportion we would like to include in the subset of data is {desired_edge_percentage:.3f}.")
...
if desired_edge_percentage is not None:
    ...
    dict_split_trials = TrialDistribution.truncate_edge_vs_central_trials_given_percentage(
        data_adaptive, xref_bounds, desired_edge_percentage, 
...
variable_names = ['sim_file_name', 'data','gt_file_name','gt_model_pred',
                  'desired_edge_percentage', 'xref_bounds',
                  'data_sobol', 'data_adaptive','dict_split_trials',
                  'nTrials_total', 'nTrials_inclusion', 'opt_params', 
                  'model','btst_seed','flag_btst', 'grid', 'target_pC']
...
if flag_running_on_hpc:
    for idx, nTrials in enumerate(nTrials_inclusion):
        ...
        output_fileDir_fits_idx = os.path.join(base_dir, 'hpc_sweeps', 'trial_efficiency',
                                               f"{ndims}D_gtCIE", 
        ...
        if desired_edge_percentage is None:
            ...
            data_subset, data_vis = TrialDistribution.select_proportion_data(
                [data_sobol, dict_split_trials['data_edge_trunc'], dict_split_trials['data_central_trunc']],
                split_trial
            )
        ...
        for flag_btst_AEPsych, ll in zip(flag_btst, btst_seed):
            ...
            if flag_btst_AEPsych:
                ...
                xref_btst, x1_btst, y_btst, btst_indices = \
                    load_expt_data.bootstrap_AEPsych_data(\
                    data_subset[1], data_subset[2], data_subset[0], 
                    trials_split= split_trial_idx, seed=ll)
            else:
            ...
            W_est, iters, objhist = optim.optimize_posterior(
                W_init, data_btst, model, OPT_KEY,
            ...
            #print the results
            print(f"Percent of edge trials: {desired_edge_percentage}; "+\
            ...
            append_variable_names = ['data_vis', 'split_trial_idx',
                                     'data_subset_copy','data_btst', 'btst_indices',
                                     'random_seeds', 'W_INIT_KEY', 'OPT_KEY', 'W_init',
                                     'W_est', 'iters', 'objhist', 'Sigmas_noise_grid', 
                                     'model_pred_Wishart', 'BWD', 'BWD_sum','BWD_avg',
                                     'BWD_min', 'BWD_max']
...
if not flag_running_on_hpc:
    ...
    BWD_results_list, params_ell_list, varying_nTrials, loaded_percentages, \
        modelpred_list, vars_dict_list = [], [], [], [],[], []

    while True:
        try:        
            # Prompt user to select one or more trial efficiency result files
            # and load corresponding BWD summary and model predictions
            BWD_results, input_fileDir_fits_set,vars_dict_set = \
                TrialDistribution.load_trial_efficiency_results()
            ...
            varying_nTrials_temp = extract_between_patterns(
                input_fileDir_fits_set, 
                str_prefix='TrialEfficiency_',
                str_suffix='trials'
            )
            ...
            loaded_percentages_temp = extract_between_patterns(
                [input_fileDir_fits_set[-1]],
                str_prefix='percentage_',
                str_suffix='/TrialEfficiency'
            )
    ...
    output_sumFig_dir = os.path.dirname(input_fileDir_fits_last.replace('ModelFitting_DataFiles',
    ...
    for i, bwd in enumerate(BWD_results_list):
        label = f"{loaded_percentages[i]}" if loaded_percentages[i] != 'None' \
            else f"Original: {org_edge_percentage:.2f}"
        
        BWD_vis_settings = replace(BWD_vis_settings, lc=colors_rgb[i],
                                   label_line=label, ylim=[1, 5])
        
        BWD_vis.plot_BWD_varying_nTrials(bwd['nTrials_inclusion'], 
                                         bwd['BWD_sum_all_org'],
                                         BWD_vis_settings,
                                         BWD_CI=bwd['BWD_CI'],
                                         ax=ax) 
...
if not flag_running_on_hpc:
    ...
    for i, vars_dict_i in enumerate(vars_dict_list):
        ...
        for i, (data_i, ttl_i) in enumerate(zip(data_vis_i, ['Sobol','Edge', 'Central'])):
            ...
            sampling_vis.plot_sampling(data_i[1],  # Reference points up to the nth data point
                                       data_i[2],  # Comparison points up to the nth data point
                                       settings = pltSettings_tp,
                                       ax = ax)      
            
            # Save the figure as a PDF
            fig.savefig(os.path.join(output_sumFig_dir_i, f'TrialEfficiency_{ttl_i}{nTrials_i}.pdf'), 
                        bbox_inches='tight')    
            plt.show()
...
if not flag_running_on_hpc:
    ...
    for i, vars_dict_i in enumerate(vars_dict_list):
        ...
        for d in range(nSets_nTrials_inclusion):
            ...
            pred2D_settings = replace(pred2D_settings, 
                                      title = f'Edge-to-total ratio: {loaded_percentages[i]}; trials: {varying_nTrials[i][d]}',
                ...
                for i in range(num_grid_pts_desired):
                    for j in range(num_grid_pts_desired):
                        ...
                        if i == 0 and j == 0:
                            ...
                            lbl = None
                        add_CI_ellipses(fitEll_min[i, j], fitEll_max[i, j],
                                        ax=ax_d, cm=cm, label=lbl, lw_outer = 0,
                                        alpha = 0.8)
            
            # Overlay model predictions (joint fits) onto the same axes
            wishart_pred_vis_wCI.plot_2D(vars_dict_i[d][0]['grid'], gt_ellipses= gt_ell_vis,
                                         ax=ax_d, settings=pred2D_settings)
