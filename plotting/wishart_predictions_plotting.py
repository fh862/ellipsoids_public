...
class Plot2DPredSettings(PlotSettingsBase):
    ...
    samples_c_no: Union[str, np.ndarray, List[float]] = field(default_factory=lambda: np.array([107, 142, 35]) / 255)
    samples_c_yes: Union[str, np.ndarray, List[float]] = field(default_factory=lambda: np.array([128, 0, 0]) / 255)
    gt_lc: Union[str, np.ndarray, List[float]] = 'r' 
    gt_alpha: float = 1.0
    gt_ls: str = '--'
    gt_label: str = 'Ground truths'
    gt_lw: float = 2
    ticks: Optional[np.ndarray] = None
    plane_2D: Optional[str] = None  # You can set this in code after instantiating with color_thres.plane_2D
    modelpred_lc: Optional[str] = None
    modelpred_ls: str = '-'
    modelpred_lw: float = 1
    modelpred_label: Optional[str] = None
    modelpred_alpha: float = 0.5
    sigma_lc: Union[str, np.ndarray, List[float]] = 'k' 
    sigma_ls: str = '-'
    sigma_lw: float = 1
    ...
    fig_name: str = field(default_factory=lambda: f'Fitted_isothreshold_ellipses_{datetime.now().strftime("%Y%m%d_%H%M%S")}')
...
class Plot3DPredSettings(PlotSettingsBase):
    ...
    gt_3Dproj_lc: Optional[Union[str, np.ndarray, List[float]]] = None
    gt_3Dproj_ls: str = '-'
    gt_lc: Union[str, np.ndarray, List[float]] = 'r'
    gt_alpha: float = 0.5
    gt_ls: str = '--'
    ...
    samples_alpha: float = 0.5
    samples_label: str = 'Simulated CIELab data'
    modelpred_lc: Union[str, np.ndarray, List[float]] = 'g'
    modelpred_ls: str = '-'
    modelpred_lw: float = 3
    modelpred_alpha: float = 0.5
    modelpred_projection_CI: Optional[np.ndarray] = None
    modelpred_slice_CI: Optional[np.ndarray] = None
    ...
    fig_name: str = field(default_factory=lambda: f'Fitted_isothreshold_ellipsoids_{datetime.now().strftime("%Y%m%d_%H%M%S")}')
...
class Plot3DPredHTMLSettings:
    ...
    sEll_line_color: Union[np.ndarray, List[float]] = field(default_factory=lambda: np.array([0,0,0]))
    ...
    geo_path_color: Union[np.ndarray, List[float]] = field(default_factory=lambda: np.array([0,0,0]))
...
class PlotMahaSettings(PlotSettingsBase):
    ...
    markersize: float = 50
    markercolor: Union[Tuple[float, float, float], str] = (0.3, 0.3, 0.3)
    edgecolor: Union[Tuple[float, float, float], str] = (1.0, 1.0, 1.0)
...
class PlotMCGeoSettings(PlotSettingsBase):
    ...
    diff_nBins: int = 100
    legend_off: bool = False
    anchor_legend_box: Optional[List] = None
...
class WishartPredictionsVisualization(PlottingTools):
    def __init__(self, trial_data, model, model_pred, color_thres, 
    ...
    def _find_idx_corresponding_xref(self, ref_all, x1_all, ref_slc, tol = 5e-2,
                                     y_all = None, further_sort_by_resp = False):
        """
        ...
        if further_sort_by_resp and y_all is not None:
            ...
            return [x1_all[yes_indices], x1_all[no_indices]], [ref_all[yes_indices], ref_all[no_indices]], [yes_indices, no_indices] 
        else:
    ...
    def _org_Mahalanobis_distance_trial_ID(self, trial_ID, resp, nTrials_bin_list):
        ...
        M_D_unsorted, M_D_sorted, resp_unsorted, resp_sorted, M_D_binned, resp_binned, nTrials = [], [], [], [], [], [], []
        ...
        for i in range(len(trial_type)):
            ...
            if len(M_D_sorted_i) % nTrials_bin_i != 0:
                ...
                resp_sorted_i = np.pad(resp_sorted_i.astype(float), (0, num_pad_nan), constant_values=np.nan)
    ...
    #%%
    def plot_2D(self, grid_est, settings: Plot2DPredSettings, 
                gt_ellipses = None, ax = None):
        """
        ...
        for ij in np.ndindex(*grid_est.shape[0:2]):
            ...
            if settings.visualize_samples and (self.trial_data.x1_all.shape[0] != 0):
                if settings.samples_colorcoded_resp:
                    x1_slc, *_ = self._find_idx_corresponding_xref(
                        self.trial_data.xref_all, self.trial_data.x1_all, 
                        np.reshape(grid_est[ij],(1,-1)), y_all = self.trial_data.y_all,
                    ...
                    for kk in range(2):
                        ...
                        ax.scatter(x1_slc[kk][:,0], x1_slc[kk][:,1], 
                                   color = c_kk, s = settings.samples_s, 
                                   linewidth = 0, alpha = 0.5, label = lbl_kk)  
                else:
                    x1_slc, *_ = self._find_idx_corresponding_xref(
                        self.trial_data.xref_all, self.trial_data.x1_all, 
                        np.reshape(grid_est[ij],(1,-1)))
                    
                    ax.scatter(x1_slc[:,0], x1_slc[:,1],
                               color = tuple(cm),
            ...
            if settings.visualize_model_estimatedCov:
                viz.plot_ellipse(ax, grid_est[ij], 
            ...
            if settings.visualize_model_pred:
                if settings.modelpred_lc is None: lineC = cm;
                else: lineC = settings.modelpred_lc
                # Plot the model predictions as lines.
                ax.plot(*self.model_pred.fitEll_unscaled[ij],
                        c = lineC, lw = settings.modelpred_lw,
        ...
        if not settings.legend_off:
            plt.legend(loc='lower center', bbox_to_anchor= settings.anchor_legend_box,
    ...
    #%% 
    def plot_3D(self, grid_est, settings: Plot3DPredSettings, 
                gt_covMat = None, gt_slice_2d_ellipse = None):
            ...
            for _idx, fixedPlane, _idx_varying, varyingPlanes in zip(settings.dim_indices,
                ...
                for i in range(NUM_GRID_PTS):
                    for j in range(NUM_GRID_PTS):
                        ...
                        if settings.visualize_model_pred:
                            polygon = Polygon(self.model_pred.fitEll_unscaled[ii,jj,kk][idx].T, 
                                              closed=True, fill=True, color = np.array(cm), 
                                              alpha = settings.modelpred_polygon_alpha,
                                              label = pred3D_label)
                            axes[0].add_patch(polygon)
                        else:
                            if settings.visualize_modelpred_CI and\
                                settings.modelpred_projection_CI is not None:
                                idx_max_nonan = ~np.isnan(fitEll_max[ii,jj,kk,_idx,0,:])
                                axes[0].fill(fitEll_max[ii,jj,kk,_idx,0,idx_max_nonan], 
                                        fitEll_max[ii,jj,kk,_idx,1,idx_max_nonan], 
                                        color= np.array(cm), label = CI_3D_label, 
                                        alpha = settings.modelpred_CI_alpha, lw = 0)
                                idx_min_nonan = ~np.isnan(fitEll_min[ii,jj,kk,_idx,0,:])
                                axes[0].fill(fitEll_min[ii,jj,kk,_idx,0,idx_min_nonan],
                                        fitEll_min[ii,jj,kk,_idx,1,idx_min_nonan], 
                                        color='white', lw = 0)
                        
                        #visualize the ground truth 3D projections on 2D 
                        if settings.visualize_gt:
                            if settings.gt_3Dproj_lc is None: c = np.array(cm)
                            else: c = settings.gt_3Dproj_lc
                            viz.plot_ellipse(axes[0], grid_est[ii,jj,kk, idx],
                                gt_covMat[ii,jj,kk][idx][:,idx],ls = settings.gt_3Dproj_ls,
                                color= c, lw=settings.gt_3Dproj_lw,
                                label=contour_3D_label)#np.array(cm)                                
                        ...
                        if settings.visualize_model_estimatedCov:
                            ...
                            viz.plot_ellipse(axes[0],  grid_est[ii,jj,kk, idx],
                                ...
                                alpha = settings.sigma_alpha, label = fits_label)  
                                
                            viz.plot_ellipse(axes[1],  grid_est[ii,jj,kk, idx],
                        ...
                        if settings.visualize_gt:
                            viz.plot_ellipse(axes[1], grid_est[ii,jj,kk,idx],
                                ...
                                color = settings.gt_lc, linestyle = settings.gt_ls,
                                lw=settings.gt_lw, alpha = settings.gt_alpha,
                                label = contour_2D_label)    
                            if settings.visualize_gt_proj_slice_one_plot:
                                viz.plot_ellipse(axes[1], grid_est[ii,jj,kk, idx],
                                    gt_covMat[ii,jj,kk][idx][:,idx],ls = '-',
                                    color= c, lw=settings.gt_3Dproj_lw,
                                    label=contour_3D_label)#np.array(cm)                                    
                        ...
                        if settings.visualize_model_pred:
                            ...
                            if settings.visualize_modelpred_CI and settings.modelpred_slice_CI is not None:
                                ...
                                axes[1].fill(fitEll_s_max[ii,jj,kk,_idx,0,idx_max_nonan], 
                                        fitEll_s_max[ii,jj,kk,_idx,1,idx_max_nonan], 
                                        color= np.array(cm), label = CI_2D_label, 
                                        alpha = settings.modelpred_CI_alpha, lw = 0)
                                idx_min_nonan = ~np.isnan(fitEll_s_min[ii,jj,kk,_idx,0,:])
                                axes[1].fill(fitEll_s_min[ii,jj,kk,_idx,0,idx_min_nonan],
                                        fitEll_s_min[ii,jj,kk,_idx,1,idx_min_nonan], 
                                        color='white', lw = 0)
                                
                #plot xref
                if settings.visualize_samples:
                    slc_idx_samples = np.where(np.abs(self.trial_data.xref_all[:,_idx]-(fixedRGB_val_scaled_k)) < 1e-4)
                    ...
                    for a in settings.axes_samples:
                        axes[a].scatter(x1_jnp_slc[:,0], x1_jnp_slc[:,1],\
                                        s=settings.samples_s,
                ...
                for n in range(2): #2 axes
                    ...
                    axes[n].legend(loc='lower center',bbox_to_anchor=(0.5, -0.45),
                                   fontsize = settings.fontsize)
                fig.tight_layout()
                plt.show()
                # Save the figure if the directory is set and saving is enabled.
    ...
    def plot_Mahalanobis_distance(self, resp, trial_ID, settings: PlotMahaSettings,
                                  sim_PMF = None, nTrials_bin = 30, ax = None):
        """
        ...
        for i in range(len(Maha_data['trial_type'])):
            ...
            CI_bounds = {
                '68%': [int(np.round(Maha_data['nTrials'][i] * 0.16)), 
                        int(np.round(Maha_data['nTrials'][i] * 0.84))],
                '95%': [int(np.round(Maha_data['nTrials'][i] * 0.025)),
            ...
            for c, alpha, k, (lb_idx, ub_idx) in zip(list(range(2)), [0.4, 0.2], key_list, CI_bounds_values):
                if settings.visualize_68CI_95CI[c]:
                    ...
                    ax[i].fill_betweenx([settings.ylim[0], settings.ylim[1]],
                        x_CI_lb, x_CI_ub, color=settings.CI_color, 
                        alpha=alpha, label = k + ' confidence interval', zorder = 1)
                    
            if sim_PMF is not None:
                ax[i].plot(sim_PMF['x'], sim_PMF['y'],
                           color = settings.linecolor, lw = settings.PMF_lw, 
                           label = settings.PMF_label, zorder = 3)
                
            if settings.visualize_SE:
                for p in settings.p_SE:
                    ax[i].errorbar(0, p, yerr = np.sqrt(p*(1-p)/nTrials_bin_i), 
            ...
            ax[i].scatter(Maha_data['M_D_binned'][i],  Maha_data['resp_binned'][i],
                          marker = settings.marker[i], color=settings.markercolor,
                          edgecolor=settings.edgecolor, s=settings.markersize, 
                          label = settings.label[i], zorder = 2)
        ...
        if i == (len(Maha_data['trial_type']) -1):
            ax[i].legend(loc = 'lower center',             
                         ...
                         ncol=1)  # Adjust the number of columns if needed
        ...
        if settings.visualize_hist:    
            ...
            for i in range(len(Maha_data['trial_type'])):    
                ...
                ax1[i].hist(normalized_D_M, bins = np.linspace(-0.2,0.2,30),
                            color=settings.markercolor, alpha = 0.3,
        ...
        if settings.visualize_timecourse:
            ...
            for i in range(len(Maha_data['trial_type'])):    
                ...
                xx_range = xx[-1] - xx[0]
                ax2[i].plot(xx,  Maha_data['M_D_unsorted'][i],
                            color=settings.markercolor, lw = 0.2)
                ...
                for c, alpha, k, (lb_idx, ub_idx) in zip(list(range(2)), [0.9, 0.9], key_list, CI_bounds_values):
                    if settings.visualize_68CI_95CI[c]:
                        x_CI_lb = Maha_data['M_D_sorted'][i][lb_idx]
                        x_CI_ub = Maha_data['M_D_sorted'][i][ub_idx]
                        ax2[i].fill_betweenx([x_CI_lb, x_CI_ub], xx[0]-xx_range*0.05,  
                                             xx[-1]+xx_range*0.05, color=settings.CI_color, 
...
def add_CI_ellipses(ell_min, ell_max, ax=None, cm='k', alpha = 0.9, label=None,lw_outer = 0, lw_inner = 0):
    ...
    ax.fill(ell_max[0, idx_max_nonan], ell_max[1, idx_max_nonan], color=cm,
            alpha= alpha, lw=lw_outer, label=label)
    ...
    ax.fill(ell_min[0, idx_min_nonan], ell_min[1, idx_min_nonan], color='white', lw=lw_inner)
...
class WishartPredictionsVisualization_html():
    ...
    def add_plane_quad(self, fig, corners_3x4):
        ...
        fig.add_trace(go.Mesh3d(
            x=P[:,0], y=P[:,1], z=P[:,2],
            i=[0, 0], j=[1, 2], k=[2, 3],            # two triangles: (0,1,2) and (0,2,3)
        ...
        fig.add_trace(go.Scatter3d(
            ...
            mode="lines", line=dict(color=edge, width= self.st.isoluminant_plane_lw),
            hoverinfo= self.st.plane_hover_info, showlegend=False
        ))
        return fig
    ...
    def apply_3d_layout(self, fig):
        ...
        fig.update_layout(
            ...
            scene_camera=dict(
                ...
                center=dict(x=self.st.camera_center[0], y=self.st.camera_center[1], z=self.st.camera_center[2]),
                up=dict(x=self.st.camera_up[0], y=self.st.camera_up[1], z=self.st.camera_up[2]),
            ),
...
class WishartPredictionsGeoVisualization(PlottingTools):
    ...
    def __init__(self, ref_W, comp1_W, comp2_W_varying, settings: PlotSettingsBase,
                 color_thres = None, save_fig=False, save_format = 'png'):
        """
        """
        super().__init__(settings, save_fig, save_format)
        self.ref_W = ref_W
    ...
    def _scatter(self, ax, Z, color, label, settings):
        ...
        """
        ax.scatter(*Z.T, c=color, s=settings.samples_size,
                   label=label, alpha=settings.samples_alpha)
    ...
    def _plot_surface3d(self, ax, surf, color, settings):
        ...
        ax.plot_surface(X, Y, Z,
                        ...
                        antialiased=True, alpha=settings.surf_alpha)     
    ...
    def _compute_fig_bounds(self, z0, z2, z1=None):
        ...
        zb = [z0[0], z2[-1], z2[0]]
        if z1 is not None: zb.append(z1[0])
        zbd = np.vstack(zb)
    ...
    def _plot_2D_predicted_geodesic_paths(self, z0, z2, paths_z0z2, ell, 
                                          settings: PlotMCGeoSettings,
                                          z1 = None, paths_z0z1 = None):
        """
        ...
        for a in range(self.nLevels_comp2):            
            ...
            if z1 is not None: self._plot_paths(ax, paths_z0z1[a], settings)
            ...
            ax.set_xlim(*new_lims[0]); ax.set_ylim(*new_lims[1])
    ...
    def _plot_3D_predicted_geodesic_paths(self, z0, z2, paths_z0z2, ell, 
                                          settings: PlotMCGeoSettings,
                                          z1=None, paths_z0z1=None):
        """
        ...
        for a in range(self.nLevels_comp2):
            ...
            ax.set_xlim(*new_lims[0]); ax.set_ylim(*new_lims[1]); ax.set_zlim(*new_lims[2]);
            ax.set_xticks(np.linspace(*new_lims[0], 4)) 
            ax.set_yticks(np.linspace(*new_lims[1], 4))
    ...
    def plot_predicted_geodesic_paths(self, z0, z2, paths_z0z2, ell, 
                                      settings: PlotMCGeoSettings, z1 = None,
                                      paths_z0z1 = None):
        """
        ...
        """
        if self.ndims == 2:
            self._plot_2D_predicted_geodesic_paths(z0, z2, paths_z0z2, ell,  
                                                   settings, z1, paths_z0z1)
        elif self.ndims == 3:
            self._plot_3D_predicted_geodesic_paths(z0, z2, paths_z0z2, ell,
                                                   settings, z1, paths_z0z1)
        else:
    ...
    def plot_diff_geodist(self, dists_z0z2, dists_z0z1, settings: PlotMCGeoSettings):
        ...
        for a in range(self.nLevels_comp2):
            ...
            if settings.flag_show_cond_title:
                title_vals = self._use_refW_as_title()
                ax.set_title(f'Ref = [{title_vals}], index of Comp#2: {a}, pComp2 = {pComp2:.2f}'+\
